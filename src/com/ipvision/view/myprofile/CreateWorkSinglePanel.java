...
import com.ipvision.service.AddWorkInfo;
import com.ipvision.service.DeleteWorkInfo;
import com.ipvision.service.aboutme.UpdateAddedWorkInfo;
 ...
 */
public class CreateWorkSinglePanel extends JPanel implements ActionListener {
    ...
    private JPanel companyCityShowPanel, positionDurationShowPanel, descriptionShowPanel, toTimePanel, holder, wrapperPnl, mainPane;
...
//    private JMenuItem deleteInfo, editInfo;
    ...
    public CreateWorkSinglePanel getInstance() {
        return instance;
    }

    public CreateWorkSinglePanel() {
        this.instance = this;
    ...
    public void init() {
        ...
        companyField = DesignClasses.makeTextFieldLimitedTextSize("", 200, 20, 200);
        ...
        positionField = DesignClasses.makeTextFieldLimitedTextSize("", 200, 20, 200);
        ...
        cityField = DesignClasses.makeTextFieldLimitedTextSize("", 200, 20, 200);
        ...
        //  toTimeNamePanel.add(DesignClasses.makeJLabel_normal("to", 14, DefaultSettings.BLACK_FONT), BorderLayout.CENTER);
...
//        editInfo.addActionListener(this);
        ...
        //System.out.println("njhsghsadge" + fromTime + " lafjkajs " + toTime);
            ...
            if (companyField.getText().trim().length() < 1 || cityField.getText().trim().length() < 1 || positionField.getText().trim().length() < 1) {
                ...
                if (HelperMethods.hasDigit(cityField.getText()) || HelperMethods.hasDigit(positionField.getText())) {
                    ...
                    if (!isCurrentWork && datePanel1.getDateinMillisecs() >= datePanel2.getDateinMillisecs()) {
                        ...
                        errorLabel.setText("");
                        GuiRingID.getInstance().getMainRight().getMyProfilePanel().getMyProfileAbout().workNew.setVisible(false);
                        new AddWorkInfo(instance).start();
...
//                if (Character.isDigit(companyField.getText().charAt(0))) {
//                    errorLabel.setText("Company name cannot start with digit");
//                } else {
                    if (HelperMethods.hasDigit(cityField.getText()) || HelperMethods.hasDigit(positionField.getText())) {
                        errorLabel.setText("City or Position cannot contain digits");
                    } else {
                        if (!isCurrentWork && datePanel1.getDateinMillisecs() >= datePanel2.getDateinMillisecs()) {
                            ...
                            errorLabel.setText("");
                            new UpdateAddedWorkInfo(instance).start();
                            //companyField.getText(), positionField.getText(), cityField.getText(), descArea.getText(), datePanel1.getDateinMillisecs(), datePanel2.getDateinMillisecs(), errorLabel).start();
                        }
                    }
                }
            ...
            //if (GuiRingID.getInstance().getMainRight().getMyProfilePanel().getMyProfileAbout().createWorkSinglePanelInstance != null) {
            //GuiRingID.getInstance().getMainRight().getMyProfilePanel().getMyProfileAbout().createWorkSinglePanelInstance = null;
            //GuiRingID.getInstance().getMainRight().getMyProfilePanel().getMyProfileAbout().workShort.setVisible(true);
            GuiRingID.getInstance().getMainRight().getMyProfilePanel().getMyProfileAbout().workNew.setVisible(false);
            // } 
            //else {
//                GuiRingID.getInstance().getMainRight().getMyProfilePanel().getMyProfileAbout().getWorkPanel().getWorkDetailsPanel().remove(0);
//                GuiRingID.getInstance().getMainRight().getMyProfilePanel().getMyProfileAbout().getWorkPanel().getWorkDetailsPanel().revalidate();
//                GuiRingID.getInstance().getMainRight().getMyProfilePanel().getMyProfileAbout().getWorkPanel().getWorkDetailsPanel().repaint();
//                GuiRingID.getInstance().getMainRight().getMyProfilePanel().getMyProfileAbout().getWorkPanel().setUnsetWorkBorder();
//            }
...
//            HelperMethods.showConfirmationDialogMessage("Are you sure to remove this work information?");
//            if (JOptionPanelBasics.YES_NO) {
//                new DeleteWorkInfo(workInfoID, errorLabel).start();                                  //new FeedDeleteCommentRequest(nfid, cmdid, AppConstants.TYPE_DELETE_STATUS_COMMENT).start();
//            }
//        }
    }
}
