...
import com.ipvision.service.AddEducationInfo;
import com.ipvision.service.DeleteEducationInfo;
import com.ipvision.service.aboutme.UpdateAddedEducationInfo;
 ...
 */
public class CreateEducationSinglePanel extends JPanel implements ActionListener {

    private JPanel holder, content, toTimePanel, fromTimePanel, attendedForButtonsPanel, wrapperPnl, mainPane;
    ...
    public JRadioButton attendedClgRadioButton, attendedGradSchlRadioButton;
    ...
    private ButtonGroup clgGrp;
    public DatePanel datePanel1, datePanel2;
    public JButton saveButton, cancelButton;
    public JButton cancelButtoninUpdateInfo, saveButtoninUpdateInfo;
...
//    private JMenuItem deleteInfo, editInfo;
    ...
    public Long fromTime, toTime, educationInfoID;
    public int type_int;  //0 for school,1 for clg
    public JPanel degreePanel;

    public CreateEducationSinglePanel getInstance() {
        return instance;
    }

    public CreateEducationSinglePanel(int type_int) {
        this.instance = this;
    ...
    public void init() {
        ...
        if (type_int == 0) {
            schoolUI("", true, true, "", null, null, false);
        } else {
            clgUI("", true, true, "", null, null, "", true, false);
        }
    }
...
//////////////////
    ...
    public void editEducationSinglePanel() {
        ...
        c1.anchor = GridBagConstraints.WEST;
//        if (type_int == 0) {
//            schoolUI(institutionTextField.getText(), currentlyStudying.isSelected(), graduated.isSelected(), descArea.getText(), datePanel1.getDateinMillisecs(), datePanel2.getDateinMillisecs(), true);
//        } else {
//            clgUI(institutionTextField.getText(), currentlyStudying.isSelected(), graduated.isSelected(), descArea.getText(), datePanel1.getDateinMillisecs(), datePanel2.getDateinMillisecs(),
//                    concentrationTextField.getText(), attendedClgRadioButton.isSelected(), true);
//        }
        if (isSchool) {
...
///////////////////////////////////////time
    ...
    public void clgUI(String ins, boolean curr, boolean grad, String desc, Long selectedfromTime, Long selectedtoTime, String cns, boolean gc, boolean ed) {
        ...
        clgGrp = new ButtonGroup();
        clgGrp.add(attendedClgRadioButton);
        clgGrp.add(attendedGradSchlRadioButton);
...
//    return (
//            (institutionTextField.getText().trim().length()>1) && (descArea.getText().trim().length()>1) &&
//            (isSchool||(!isSchool && concentrationTextField.getText().trim().length()>1&&(attendedClgRadioButton.isSelected()||attendedGradSchlRadioButton.isSelected()&&degr) ))
//            );
//    }
    @Override
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == saveButton) {
            if (institutionTextField.getText().trim().length() < 1) {
                ...
                if (!currentlyStudying.isSelected() && datePanel1.getDateinMillisecs() >= datePanel2.getDateinMillisecs()) {
                    ...
                    errorLabel.setText("");
                    new AddEducationInfo(instance).start();
                    //companyField.getText(), positionField.getText(), cityField.getText(), descArea.getText(), datePanel1.getDateinMillisecs(), datePanel2.getDateinMillisecs(), errorLabel).start();
                }
            }
        } else if (e.getSource() == cancelButton) {
            if (isSchool) {
                GuiRingID.getInstance().getMainRight().getMyProfilePanel().getMyProfileAbout().getEducationPanel().getSchoolDetailsPanel().remove(instance);
                GuiRingID.getInstance().getMainRight().getMyProfilePanel().getMyProfileAbout().getEducationPanel().getSchoolDetailsPanel().revalidate();
                GuiRingID.getInstance().getMainRight().getMyProfilePanel().getMyProfileAbout().getEducationPanel().getSchoolDetailsPanel().repaint();
            } else {
                GuiRingID.getInstance().getMainRight().getMyProfilePanel().getMyProfileAbout().getEducationPanel().getClgDetailsPanel().remove(instance);
                GuiRingID.getInstance().getMainRight().getMyProfilePanel().getMyProfileAbout().getEducationPanel().getClgDetailsPanel().revalidate();
                GuiRingID.getInstance().getMainRight().getMyProfilePanel().getMyProfileAbout().getEducationPanel().getClgDetailsPanel().repaint();
                if (GuiRingID.getInstance().getMainRight().getMyProfilePanel().getMyProfileAbout().getEducationPanel().getClgDetailsPanel().getComponentCount() == 0) {
            ...
            if (sameValues()) {
                ...
                if (institutionTextField.getText().trim().length() < 1) {
                    ...
                    if (!currentlyStudying.isSelected() && datePanel1.getDateinMillisecs() >= datePanel2.getDateinMillisecs()) {
                        ...
                        errorLabel.setText("");
                        new UpdateAddedEducationInfo(instance).start();
                        //companyField.getText(), positionField.getText(), cityField.getText(), descArea.getText(), datePanel1.getDateinMillisecs(), datePanel2.getDateinMillisecs(), errorLabel).start();
                    }
                }
            }
            ...
            showUpdatedInfo();
        } else if (e.getSource() == currentlyStudying) {
            if (!currentlyStudying.isSelected()) {
                currnt.setVisible(false);
                toTimePanel.setVisible(true);
            } else {
