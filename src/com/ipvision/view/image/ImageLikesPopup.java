 ...
 * @author Faiz
 */
public class ImageLikesPopup extends LikesJDialogBasics {

    private JLabel loading = DesignClasses.loadingLable(true);
    org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(ImageLikesPopup.class);
    ...
    private int count = 0;
    private static ImageLikesPopup imageLikesPopup;

    public static ImageLikesPopup getInstance() {
        return imageLikesPopup;
    }

    public void setVisible(JComponent com, int type_int) {
        imageLikesPopup = this;
    ...
    public ImageLikesPopup(JComponent locationRelativeTo, AddImageCaptionCommentsPanel addImageCaptionCommentsPanel) {
        this.imageLikesPopup = this;
        ...
        if (addImageCaptionCommentsPanel.getImageLikes() != null
                && addImageCaptionCommentsPanel.getImageLikes().size() > 0) {
            loadLikes();
        }
    }
...
//            frienImageLabel.setIcon(new ImageIcon(img));
    ...
    public void addLikesFromServer(ArrayList<SingleMemberInCircleDto> likesList) {
        ...
        if (addImageCaptionCommentsPanel.getImageLikes().size() < addImageCaptionCommentsPanel.imageDTo.getNl()) {
            addSeeMore(addImageCaptionCommentsPanel.getImageLikes().size());
        }
        content.revalidate();
    }
    ...
    private void loadLikes() {
        ...
        likes = addImageCaptionCommentsPanel.getImageLikes();
        ...
        if (likes.size() < addImageCaptionCommentsPanel.imageDTo.getNl()) {
        ...
        panel.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getSource() == panelSeeMore) {
                    removeSeeMore();
                    addLoading();
                    new CommentsLikes(addImageCaptionCommentsPanel.imageDTo.getImageId(), AppConstants.TYPE_LIKES_FOR_IMAGE, count).start();
    ...
    /*class LoadLikes extends Thread {
     ...
     likes = addImageCaptionCommentsPanel.getImageLikes();
     if (likes != null
     && addImageCaptionCommentsPanel.imageDTo.getNl() != null
     && likes.size() >= addImageCaptionCommentsPanel.imageDTo.getNl()) {
     break;
     } else {
     Thread.sleep(200);
     }
