 ...
 * @author Faiz
 */
public class ClearAllFeedsByFriendBook extends Thread {

    public ClearAllFeedsByFriendBook() {
        ...
        try {
            ...
            if (!NewsFeedMaps.getInstance().getSingleStatusPanelInFriendNewsFeed().isEmpty()) {
                for (String user : MyfnfHashMaps.getInstance().getMyFriendProfiles().keySet()) {
                    ...
                    if (profile.myFriendNewsFeedPanel != null && !profile.myFriendNewsFeedPanel.isDisplayable()) {
                        profile.myFriendNewsFeedPanel.removeAllStatus();
                        profile.myFriendNewsFeedPanel = null;
                    }
                    if (profile.myFriendContactListPane != null && !profile.myFriendContactListPane.isDisplayable()) {
                        profile.myFriendContactListPane.removeAll();
                        profile.myFriendContactListPane = null;
                    }
                    if (profile.myFriendAlbumPanel != null && !profile.myFriendAlbumPanel.isDisplayable()) {
                        profile.myFriendAlbumPanel.removeAll();
                        profile.myFriendAlbumPanel = null;
                    }
                    if (profile.myFriendAboutPanel != null && !profile.myFriendAboutPanel.isDisplayable()) {
