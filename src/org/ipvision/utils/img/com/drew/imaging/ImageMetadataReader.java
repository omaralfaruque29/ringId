...
import org.ipvision.utils.img.com.drew.imaging.jpeg.JpegMetadataReader;
...
/**
 * Obtains metadata from all supported file formats, including JPEG, RAW
 * (NEF/CRw/CR2) and TIFF. If the caller knows their file to be of a particular
 * type, they may prefer to use the dedicated MetadataReader directly
 * (<code>JpegMetadataReader</code> for Jpeg files, or
 * <code>TiffMetadataReader</code> for TIFF and RAW files). The dedicated
 * readers offer a very slight performance improvement, though for most
 * scenarios it is simpler, more convenient and more robust to use this class.
 *
...
public class ImageMetadataReader {
    ...
    /**
     * Reads metadata from an input stream. The file inputStream examined to
     * determine its type and consequently the appropriate method to extract the
     * data, though this inputStream transparent to the caller.
     *
     * @param inputStream a stream from which the image data may be read. The
     * stream must be positioned at the beginning of the image data.
     * @return a populated Metadata object containing directories of tags with
     * values and any processing errors.
    ...
    /**
     * Reads metadata from a file. The file is examined to determine its type
     * and consequently the appropriate method to extract the data, though this
    ...
    /**
     * An application entry point. Takes the name of one or more files as
     * arguments and prints the contents of all metadata directories to
     * System.out. If <code>/thumb</code> is passed, then any thumbnail data
     * will be written to a file with name of the input file having '.thumb.jpg'
     * appended.
     *
